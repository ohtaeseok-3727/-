...
void PostProcessing::ReleaseTargets()
{
    ...
    glDeleteTextures(2, m_BlurScene);
    glDeleteTextures(2, m_BlurBloom);
    for (int i = 0; i < 2; ++i)
        m_BlurFbo[i] = m_BlurScene[i] = m_BlurBloom[i] = 0;
    m_Ready = false;
}
...
bool PostProcessing::Resize(int width, int height)
{
    ...
    bool complete = m_SceneFbo && m_SceneTexture &&
                    glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE;
    glGenFramebuffers(2, m_BlurFbo);
    glGenTextures(2, m_BlurScene);
    glGenTextures(2, m_BlurBloom);
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_BlurScene[i], 0);
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, m_BlurBloom[i], 0);
...
void PostProcessing::Composite()
{
    ...
    BindSources(m_SceneTexture,
                needsBlur ? m_BlurScene[0] : m_SceneTexture,
    ...
    glUniform1f(m_VignetteStrength,
                settings.vignette ? Clamp(settings.vignetteStrength, 0.f, .95f) : 0.f);
    glUniform1f(m_EdgeStrength,
                settings.edgeBlur ? Clamp(settings.edgeBlurStrength, 0.f, 1.f) : 0.f);
    DrawPass(2);
...
{
    if (!m_Ready)
        return "POST FX unavailable - direct rendering";
