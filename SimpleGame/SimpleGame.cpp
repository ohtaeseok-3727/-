...
namespace
{
    ...
    {
        // Ctrl+A..Z arrive as control characters; keep movement and key-up paired.
        if (key >= 1 && key <= 26)
        {
            return static_cast<unsigned char>('a' + key - 1);
        }
    ...
    {
        keys[NormalizeKey(key)] = false;
    }

    void SpecialDown(int key, int, int)
    {
        if (key < 0 || key >= 256 || arrows[key])
            return;
        arrows[key] = true;
        if (key == GLUT_KEY_PAGE_UP)
        {
            world.ChangeInventoryPage(-1);
        }
    ...
    void Timer(int)
    {
        ...
        if (!focused)
            ClearInput();
        float dx =
            float(keys['d'] || arrows[GLUT_KEY_RIGHT]) - float(keys['a'] || arrows[GLUT_KEY_LEFT]);
        float dy =
            float(keys['s'] || arrows[GLUT_KEY_DOWN]) - float(keys['w'] || arrows[GLUT_KEY_UP]);
        bool sprint = focused && (GetAsyncKeyState(VK_SHIFT) & 0x8000) != 0;
        world.SetPaused(paused || !focused);
        const bool attackPressed = attackInput.Poll(focused && !paused && world.Started());
        world.Update(dt, dx, dy, sprint, attackPressed);
        glutPostRedisplay();
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr
            << "Renderer or character sprite initialization failed. See the asset error above.\n";
        renderer.reset();
        glutDestroyWindow(window);
        return 1;
    }
